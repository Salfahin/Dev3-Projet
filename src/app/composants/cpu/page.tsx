...
export default function CPUPage() {
  ...
  if (loading) return <p className="p-4">Fetching CPUs…</p>

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">All our CPUs</h1>
      {cpus.length === 0 && (
        <p className="text-gray-500">{":( No CPU found"}</p>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cpus.map(cpu => (
          <div key={cpu.name} className="bg-[#ee9b69] border rounded-xl p-4 shadow hover:shadow-lg transition">
            <h2 className="text-xl font-semibold">{cpu.name}</h2>
            <p className="text-sm text-gray-500">{cpu.manufacturer}</p>
            <div className="mt-2 text-sm">
              <p><strong>Cores:</strong> {cpu.specs.cores}</p>
              <p><strong>Threads :</strong> {cpu.specs.threads}</p>
              <p><strong>Price:</strong> {cpu.price} €</p>
            </div>
            <button className="mt-4 bg-[#79c39e] text-white px-4 py-2 rounded hover:bg-[#fffff] border border-[#383431]">
              Add to config
            </button>
          </div>
        ))}
